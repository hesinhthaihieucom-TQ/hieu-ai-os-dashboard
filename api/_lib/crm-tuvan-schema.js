    ...
    properties: {
      ...
      khach_hang: {
        ...
        properties: {
          ...
          nhom_nhu_cau: { type: 'array', items: { type: 'string' }, description: 'Mảng ĐẦY ĐỦ sau khi cập nhật — nếu HỒ SƠ KHÁCH ĐÃ CÓ đã có mục nào còn đúng, PHẢI giữ lại rồi mới thêm mục mới phát hiện lần này, không chỉ liệt kê mỗi mục nhắc tới trong đoạn chat đang đọc.' },
          nhu_cau_cu_the: { type: 'string' },
          van_de_noi_dau: { type: 'string' },
          giai_doan: { type: 'string' },
          do_nong: { type: 'string', enum: ['Nóng','Ấm','Lạnh'] },
          rao_can: { type: 'array', items: { type: 'string' }, description: 'Mảng ĐẦY ĐỦ sau khi cập nhật — cùng nguyên tắc giữ-lại-rồi-thêm-mới như nhom_nhu_cau, chỉ bỏ mục cũ nếu chat lần này cho thấy rào cản đó đã được gỡ.' },
