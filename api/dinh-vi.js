...
- NGẮN GỌN LÀ ƯU TIÊN SỐ 1 — người đọc lướt trên điện thoại, không đọc đoạn văn dài. Mỗi trường chỉ giữ đúng 2-3 ý cốt lõi nhất, cắt hết phần diễn giải/ví dụ phụ không cần thiết. Thà thiếu chi tiết phụ còn hơn dài dòng.
- KHÔNG viết dồn thành 1 đoạn văn dài dính liền — mỗi ý/luận điểm riêng phải được tách ra thành 1 đoạn mới bằng cách xuống dòng thật (như đang gõ Enter), để người đọc lướt nhanh vẫn nắm được, giống trình bày từng gạch ý chứ không phải văn xuôi liên tục. TUYỆT ĐỐI không gõ ra chữ "\n" như 1 ký hiệu trong nội dung câu trả lời — phải là xuống dòng thật, không phải in ra ký tự đó.
...
async function callClaude({ apiKey, system, userContent, tool }) {
  ...
  // sớm để người dùng biết mà thử lại. max_tokens=16000 (nâng 2026-09-01) cần nhiều thời gian sinh
  // chữ hơn — trần riêng 250s cho lệnh gọi AI này, vẫn còn dư địa dưới trần 300s của Vercel.
  ...
  try {
    resp = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        // 8000 từng đủ nhưng cùng công thức đã bị cắt giữa chừng ở san-pham-so (outline nhiều mục) —
...
module.exports = async (req, res) => {
  ...
  // Lượt 1 + Lượt 2 tính GỘP CHUNG thành 1 lượt dùng — Lượt 2 chạy ngầm ngay sau Lượt 1, cùng 1
  // hành động "làm Định Vị" trong mắt người dùng, không phải 2 việc tách rời. Chỉ trừ/hoàn lượt ở
