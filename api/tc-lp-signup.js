// Tạo tài khoản Sổ Dòng Tiền Tâm Thức THẲNG từ Landing Page bán hàng (tai-chinh/lp/) — chị Quỳnh
// yêu cầu 2026-09-16: gõ thông tin xong ra ngay mã QR thanh toán, không cần chuyển qua màn hình app
// trước (giảm bớt cảm giác dài dòng khiến khách không chốt). Landing page là trang tĩnh KHÔNG có
// phiên đăng nhập, nên không dùng supabaseClient.auth.signUp() bình thường của app được (client cần
// đọc lại profiles.ref_code ngay sau đó qua RLS mà RLS yêu cầu đã có session) — thay vào đó tạo qua
// Admin API (service role, bỏ qua RLS) NGAY TẠI SERVER, đọc luôn ref_code trả về cho client dựng mã
// QR. email_confirm:true để kích hoạt được ngay (không bắt khách xác nhận email mới thanh toán được
// — vào đúng lúc khách đã sẵn sàng trả tiền, thêm 1 bước xác nhận email dễ làm rơi mất họ).
...
module.exports = async (req, res) => {
  ...
  if (!createResp.ok) {
    ...
    const msg = /already.*registered|already.*exists|duplicate/i.test(rawMsg)
      ? 'Email này đã có tài khoản Sổ Dòng Tiền Tâm Thức rồi — vào app và đăng nhập để lấy mã thanh toán.'
