...
function render(container, ctx){
  ...
  function html(){
    return `
      ...
      <div class="card">
        ...
        <div class="btn-row"><button class="btn" data-action="score" ${state.loading?'disabled':''}>${state.loading?'Đang chấm…':'Chấm điểm hook này'}</button> <span style="font-size:12.5px;color:var(--ink-soft);align-self:center;">(tốn 1 lượt AI)</span></div>
      ...
      ${state.history.length ? `<div style="margin-top:28px;"><h3 style="font-family:'IBM Plex Mono',monospace;font-size:14.5px;color:var(--ink-soft);text-transform:uppercase;letter-spacing:.05em;margin-bottom:12px;">Lịch sử chấm gần đây</h3>
        ${state.history.map(h=>`<div class="list-item"><div class="txt"><div class="meta">${new Date(h.created_at).toLocaleDateString('vi-VN')} · ${h.result.diem_tong}/100</div>${esc(h.hook_text)}</div><span style="color:var(--danger);cursor:pointer;font-size:13.5px;white-space:nowrap;" data-del-history="${h.id}">Xoá</span></div>`).join('')}
      </div>` : ''}
    `;
  }
  ...
  function resultHtml(){
    ...
    return `
      ...
      <div class="btn-row"><span class="btn-ghost btn" data-action="improve" ${state.improving?'disabled':''}>${state.improving?'Đang viết lại…':'Viết lại 5 hook mới theo góp ý này →'}</span> <span style="font-size:12.5px;color:var(--ink-soft);align-self:center;">(tốn 1 lượt AI)</span></div>
