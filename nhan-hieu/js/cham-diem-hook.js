...
function render(container, ctx){
  ...
  function html(){
    return `
      ...
      <div class="card">
        ...
        <div class="btn-row"><button class="btn" data-action="score" ${state.loading?'disabled':''}>${state.loading?'Đang chấm…':'Chấm điểm hook này'}</button> <span style="font-size:11px;color:var(--ink-soft);align-self:center;">(tốn 1 lượt AI)</span></div>
  ...
  function resultHtml(){
    ...
    return `
      ...
      <div class="btn-row"><span class="btn-ghost btn" data-action="improve" ${state.improving?'disabled':''}>${state.improving?'Đang viết lại…':'Viết lại 5 hook mới theo góp ý này →'}</span> <span style="font-size:11px;color:var(--ink-soft);align-self:center;">(tốn 1 lượt AI)</span></div>
