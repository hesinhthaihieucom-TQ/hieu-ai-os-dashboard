// Duyệt đánh giá Trợ Lý CRM trước khi hiện công khai ở Trang chủ (2026-09-07, "quản trị bên xây
// nhân hiệu có gì bên crm có đó") — copy đúng pattern nhan-hieu/js/quan-tri-danhgia.js, chỉ lọc
// riêng app='tro-ly-crm' trong bảng app_reviews dùng chung.
(function(){
function render(container, ctx){
  ...
  async function load(){
    const { data } = await ctx.supabase.from('app_reviews').select('*').eq('app', 'tro-ly-crm').order('created_at', { ascending:false }).limit(100);
    state.reviews = data || [];
  }
